...
<p class="wms_widget_text">
	<label for="<?php echo $this->get_field_name( 'playlist_id' ); ?>"><?php _e( 'YouTube Playlist ID' )?> <small><?php _e('e.g.: PLsRNoUx8w3rNhkOfRBau4qOeHXVMm92NB')?></small>
		<input class="widefat videowall-playlist-id" id="<?php echo $this->get_field_id( 'playlist_id' ); ?>" name="<?php echo $this->get_field_name( 'playlist_id' ); ?>" type="text" value="<?php echo $instance['playlist_id']; ?>" />
	</label>
</p>
...
<p class="wms_widget_checkbox">
	<input class="videowall-hide-intro" id="<?php echo $this->get_field_id('hide_intro'); ?>" name="<?php echo $this->get_field_name('hide_intro'); ?>" type="checkbox" value="1" <?php checked( '1', $instance['hide_intro'] ); ?>/>
	<label for="<?php echo $this->get_field_name('hide_intro'); ?>"><?php _e('Hide Intro Block'); ?>?</label>
</p>

<fieldset class="meerkat-videowall-fieldset">
	<legend><?php _e( 'Intro Block' )?></legend>
	
	<label for="<?php echo $this->get_field_name( 'content' ); ?>"><?php _e( 'Content Blurb' )?>
    	<br /><small><?php _e('Keep this short!')?></small>
    	<script type="text/javascript">
    	jQuery(document).ready(function(){
    	    jQuery('#<?php echo $this->get_field_id( 'content' ); ?>').parents('form').find('input.widget-control-save').click(function(){
    	        tinymce.get('<?php echo $this->get_field_id( 'content' ); ?>').save();
    	        tinymce.execCommand('mceRemoveControl', false, '<?php echo $this->get_field_id( 'content' ); ?>');
    	    });
        });
        </script>
    	<?php 
    	   $editor_settings = array('textarea_name' => $this->get_field_name( 'content' ), 'teeny' => true, 'media_buttons' => false); 
    	   wp_editor( $instance['content'], $this->get_field_id( 'content' ), $settings = $editor_settings );
    	?>
	</label>

	<?php
	if ( $this->use_old_uploader() ) {
		include( $this->image_widget_getTemplateHierarchy( 'image-widget-admin.deprecated' ) );
	} else {
		include( $this->image_widget_getTemplateHierarchy( 'image-widget-admin' ) );
	}
	?>
</fieldset>
